...
Weapon property ccBGSSSE001_FishingRodAlikriWeap auto
Weapon property ccBGSSSE001_FishingRodArgonianWeap auto
Weapon property ccBGSSSE001_FishingRodDwarvenWeap auto

Activator property ccBGSSSE001_FishingRodFXImperial auto
Activator property ccBGSSSE001_FishingRodFXAlikri auto
Activator property ccBGSSSE001_FishingRodFXArgonian auto
Activator property ccBGSSSE001_FishingRodFXDwarven auto
...
Keyword property FEF_SummonsClear auto
Keyword property FEF_SummonsRain auto
...
{ The weather to force to when wearing equipment that summons clear skies. }
...
;/ 
 ...
 / ==========================
 /;
int FEF_CurrentBonusType = -1
Activator FEF_CurrentActivator = None
FEFBiomeHolderScript FEFCurrentFishingBiome
...
function StartPlayerInteraction(ccBGSSSE001_FishingActScript akFishingSupplies, bool abContinueFishing = false)
	...
	FEFCurrentFishingBiome = akFishingSupplies as FEFBiomeHolderScript
	ResetSystem()
...
ccBGSSSE001_CatchData function GetNextCatchData()
	...
	if catchTypeRoll >= catchChanceFish
		...
		int biomecheck = FEFCurrentFishingBiome.BiomeNormalFormList.GetSize()
		int biomeraincheck = FEFCurrentFishingBiome.BiomeRainFormList.GetSize()
		if biomecheck == 0 && biomeraincheck == 0
			...
			if GetInRain() && biomeraincheck > 0
				catchDataList = FEFCurrentFishingBiome.BiomeRainFormList
			else
				catchDataList = FEFCurrentFishingBiome.BiomeNormalFormList
			endif
		endif
...
ObjectReference function PlaceFishingRodActivator(int aiRodType)
	...
	ObjectReference fishingRodRef = None
	fishingRodPlacementMarker.MoveTo(fishingRodPlacementMarker, afZOffset = -600.0)
	if !FEF_CurrentActivator
		fishingRodRef = fishingRodPlacementMarker.PlaceAtMe(ccBGSSSE001_FishingRodFXActivators.GetAt(aiRodType))
	else
		fishingRodRef = fishingRodPlacementMarker.PlaceAtMe(FEF_CurrentActivator)
	endif
...
bool function IsCatchSuccessful()
	if IsFishCatchType(nextCatchData.getCatchType())
		...
		bool forceSpecificRod = (nextCatchData as ccBGSSSE001_FishCatchData).getRequireExactRod()
		FEFFishingRodScript requiredRodProperties
		Int requiredRodInt
		
		if requiredRod
			if requiredRod == ccBGSSSE001_FishingRodImperialWeap
				requiredRodInt = 0
			elseif requiredRod == ccBGSSSE001_FishingRodAlikriWeap
				requiredRodInt = 1
			elseif requiredRod == ccBGSSSE001_FishingRodArgonianWeap
				requiredRodInt = 2
			elseif requiredRod == ccBGSSSE001_FishingRodDwarvenWeap
				requiredRodInt = 3
			else
				requiredRodProperties = requiredRod as FEFFishingRodScript
				if requiredRodProperties && requiredRodProperties.FishingRodBonusType > -1
					requiredRodInt = requiredRodProperties.FishingRodBonusType
					;debug.notification("FEF: Successfully read the script and set the rod type to " + requiredRodInt)
				endif
			endif
		endif
		...
		if !requiredRod
			return true
		elseif (forceSpecificRod == true && (requiredRod == PlayerRef.GetEquippedWeapon() || requiredRod == PlayerRef.GetEquippedWeapon(abLeftHand = true)))
			;debug.notification("FEF: Using the exact rod as specified.")
			return true
		elseif (forceSpecificRod == true)
			;debug.notification("FEF: Not using the exact rod as specified in either hand.")
			return false
		elseif requiredRodInt != currentFishingRodType
			;debug.notification("FEF: No exact rod specified. Type mismatch. Required: " + requiredRodInt + ", Equipped: " + currentFishingRodType)
			return false
		endif
	endif
...
function TryToApplyRainWeather()
	...
	int biomecheck = FEFCurrentFishingBiome.BiomeNormalFormList.GetSize()
	int biomeraincheck = FEFCurrentFishingBiome.BiomeRainFormList.GetSize()
	weather BiomeRainWeather = FEFCurrentFishingBiome.BiomeRainWeatherOverride 
	weather BiomeClearWeather = FEFCurrentFishingBiome.BiomeClearWeatherOverride
	if (PlayerRef.WornHasKeyword(ccBGSSSE001_SummonsRain) || PlayerRef.HasMagicEffectWithKeyword(ccBGSSSE001_SummonsRain) || PlayerRef.WornHasKeyword(FEF_SummonsRain) || PlayerRef.HasMagicEffectWithKeyword(FEF_SummonsRain)) && ((biomecheck == 0 && biomeType == BIOME_TYPE_STREAM) || (biomecheck == 0 && biomeType == BIOME_TYPE_LAKE) || biomeraincheck > 0)
		; Override the weather check for fishing purposes.
		forcedRainWeather = true
		;debug.notification("FEF: forcedRainWeather set to " + forcedRainWeather)
		; Change the weather visuals if necessary.
		Weather currentWeather = Weather.GetCurrentWeather()
		if currentWeather.GetClassification() < 2 ; Rain or Snow (Widened to general precipitation since snow generally replaces rain in colder climates)
			if !previousWeather
				previousWeather = currentWeather
				;debug.notification("FEF: Should only see this once per spot.")
			endif
			if !BiomeRainWeather
				SkyrimStormRain.SetActive(true, true)
			else
				BiomeRainWeather.SetActive(true, true)
			endif
		endif
	elseif (PlayerRef.WornHasKeyword(FEF_SummonsClear) || PlayerRef.HasMagicEffectWithKeyword(FEF_SummonsClear)) && ((biomecheck == 0 && biomeType == BIOME_TYPE_STREAM) || (biomecheck == 0 && biomeType == BIOME_TYPE_LAKE) || biomeraincheck > 0)
		; Override the weather check for fishing purposes.
		forcedClearWeather = true
		;debug.notification("FEF: forcedClearWeather set to " + forcedClearWeather)
		; Change the weather visuals if necessary.
		Weather currentWeather = Weather.GetCurrentWeather()
		if currentWeather.GetClassification() != 0 ; Pleasant
			if !previousWeather
				previousWeather = currentWeather
				;debug.notification("FEF: Should only see this once per spot.")
			endif
			if !BiomeClearWeather
				SkyrimClear.SetActive(true, true)
			else
				BiomeClearWeather.SetActive(true, true)
			endif
		endif
	endif
endFunction
...
function ClearFishingSessionVariables()
	...
	FEF_CurrentBonusType = -1
	FEF_CurrentActivator = None
endFunction
...
int function GetCurrentFishingRodType()
	...
	FEFFishingRodScript FEF_currentlyEquippedRodRight = PlayerRef.GetEquippedWeapon() as FEFFishingRodScript
	if equippedWeaponRightHand
		if ccBGSSSE001_FishingRodImperialWeap && equippedWeaponRightHand == ccBGSSSE001_FishingRodImperialWeap
			FEF_CurrentActivator = ccBGSSSE001_FishingRodFXImperial
			FEF_CurrentBonusType = 0
			return FEF_CurrentBonusType
		elseif ccBGSSSE001_FishingRodAlikriWeap && equippedWeaponRightHand == ccBGSSSE001_FishingRodAlikriWeap
			FEF_CurrentActivator = ccBGSSSE001_FishingRodFXAlikri
			FEF_CurrentBonusType = 1
			return FEF_CurrentBonusType
		elseif ccBGSSSE001_FishingRodArgonianWeap && equippedWeaponRightHand == ccBGSSSE001_FishingRodArgonianWeap
			FEF_CurrentActivator = ccBGSSSE001_FishingRodFXArgonian
			FEF_CurrentBonusType = 2
			return FEF_CurrentBonusType
		elseif ccBGSSSE001_FishingRodDwarvenWeap && equippedWeaponRightHand == ccBGSSSE001_FishingRodDwarvenWeap
			FEF_CurrentActivator = ccBGSSSE001_FishingRodFXDwarven
			FEF_CurrentBonusType = 3
			return FEF_CurrentBonusType
		elseif FEF_currentlyEquippedRodRight
			FEF_CurrentActivator = FEF_currentlyEquippedRodRight.FishingRodActivator
			FEF_CurrentBonusType = FEF_currentlyEquippedRodRight.FishingRodBonusType
			return FEF_CurrentBonusType
		else
			FEF_CurrentBonusType = -1
			return FEF_CurrentBonusType
	...
	FEFFishingRodScript FEF_currentlyEquippedRodLeft = PlayerRef.GetEquippedWeapon(abLeftHand = true) as FEFFishingRodScript
	if equippedWeaponLeftHand
		...
		elseif ccBGSSSE001_FishingRodDwarvenWeap && equippedWeaponRightHand == ccBGSSSE001_FishingRodDwarvenWeap
			...
			FEF_CurrentBonusType = 3
			return FEF_CurrentBonusType
		elseif FEF_currentlyEquippedRodLeft
			FEF_CurrentActivator = FEF_currentlyEquippedRodLeft.FishingRodActivator
			FEF_CurrentBonusType = FEF_currentlyEquippedRodLeft.FishingRodBonusType
			return FEF_CurrentBonusType
		else
			FEF_CurrentBonusType = -1
			return FEF_CurrentBonusType
...
bool function GetInRain()
	...
	if forcedRainWeather
		return true
	; If using equipment that summons clear skies, we are never in rainy weather.
	elseif forcedClearWeather
		return false
	endif
...
function RestoreWeather()
	if previousWeather
		...
		previousWeather.SetActive(false, false)
		previousWeather = None
;		debug.notification("FEF: Previous Weather supposedly restored...")
	endif
endFunction
